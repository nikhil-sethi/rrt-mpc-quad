...
def main():
    ...
    state_init = np.transpose(np.array([2.5, 0., 0., 0., 0., 0., 0., 0., np.deg2rad(90), 0., 0., 0.]))
    x[:,0] = state_init


    num_points = 50
    path_points = np.array([[2.5*np.cos(theta), 2.5*np.sin(theta), 0.0] for theta in np.linspace(0,2*np.pi, num_points)])
